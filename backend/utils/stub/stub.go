...
import (
	...
	"path/filepath"
	"strings"
	"gitlab.com/project-quiz/utils/modname"
	"text/template"
...
func MakeStubs() error {
	...
	for _, f := range files {
		...
		output := bytes.Replace(input, []byte("gitlab.com/project-quiz/"), []byte("gitlab.com/project-quiz/"), -1)
		output = bytes.Replace(output, []byte("module gitlab.com/project-quiz"), []byte("module gitlab.com/project-quiz"), -1)
...
func TemplateStub(templateType string, templateName string, name string) {
	contentsBuff, err := os.ReadFile(fmt.Sprintf("./stubs/gitlab.com/project-quiz/%s/%s.go.stuba", templateType, templateName))
	if err != nil {
